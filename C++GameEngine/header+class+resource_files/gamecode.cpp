// GameCode.cpp		
/*edited by: w16014375*/
/*edited to include a loop that creates rocks for the game as well as creating the hud and spaceship.*/
...
ErrorType Game::StartOfGame()
{
   ...
   // **********************************************************************
	...
	Spaceship* temp = new Spaceship();//creates pointer to Spaceship object
	temp->initialise(& pObjectManager);//initialises Spaceship
	pObjectManager.addObject(temp);//adds object pointer to object list
	pObjectManager.canReceiveMessages(temp);//adds object to a list that can receive Messages
	
	for (int i = 1; i < 10; i++)//initialises 10 Rocks and adds them to the object list
	{
		Rock*temp2 = new Rock();
		temp2->initialise(& pObjectManager);
		pObjectManager.addObject(temp2);
		pObjectManager.canReceiveMessages(temp2);
	}

	Hud* temp3 = new Hud();//creates and adds Hud to object and receive Messages list
	temp3->initialise(& pObjectManager);
	pObjectManager.addObject(temp3);
	pObjectManager.canReceiveMessages(temp3);
...
ErrorType Game::Update()
{
	...
	pObjectManager.updateAll();//updates all objects
	pObjectManager.renderAll();//renders all objects
